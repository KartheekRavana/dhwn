...
function loadItems(row,val)
{
 ...
 <script type="text/javascript">
  ...
  <body class="skin-blue fixed" onload="checkCustomer(document.getElementById('bill_type').value)">
    <div class="wrapper">
       <!-- ---------------------------------------------------POPUP--------------------------------------------------------------------------- -->
<div class='popup'>
...
<!-- ---------------------------------------------------POPUP--------------------------------------------------------------------------- -->  
      ...
      <div class="content-wrapper">
        <!-- Content Header (Page header) -->
          ...
          $phone="";
          $CUSTOMER_ID="";
          $data = mysql_query("SELECT bill_no, customer_name, place, phone, bill_date, total, login_id, branch, other_exp, grand_total from customerreturnmain where bill_no='".$_GET["bill_no"]."' and branch='$session_branch'");
          while($info = mysql_fetch_array( $data ))
          {
          	$return_date=$info["bill_date"];
          ...
          if($customer_name=="")
          {
          	$data = mysql_query("SELECT CUSTOMER_ID, TRAN_DATE, TRAN_TIME, BILL_NO, slip_no, credit, debit, TRAN_STATUS, employee, branch, tran_id, balance, note, customer_name, tran_type, transaction_id from customer_transactions where bill_no='".$_GET["bill_no"]."' and note='RETURN'");
          	while($info = mysql_fetch_array( $data ))
          	{
          		$CUSTOMER_ID=$info["CUSTOMER_ID"];
          	}
          	$data = mysql_query("SELECT customer_name from customer where CUSTOMER_ID='$CUSTOMER_ID'");
          	while($info = mysql_fetch_array( $data ))
          	{
          		$c_customer_name=$info["customer_name"];
          	}
          }
        
          ?>
          <form role="form" onsubmit="return validate()" action="index.php?action=operations/stockreturn_update&c=stock" method="POST">
          <input type='hidden' id='data' name='data'>
           <input type='hidden' name="bill_no" id="bill_no" value="<?php echo $_GET["bill_no"]?>">
           <input type='hidden' name="tran_id" id="tran_id" value="<?php echo $_GET["tran_id"]?>">
            ...
            <div class="col-md-12">
              ...
              <div class="box box-primary">
                ...
                <tr>
                	...
                	<td>Return&nbsp;Date</td><td id='bill_date_v'><input type="date" class="form-control" id="bill_date" name="bill_date" value="<?php echo $return_date?>" style="width: 150px"> </td><td>&nbsp;&nbsp;</td>
                	<td>Customer&nbsp;Type</td><td id='bill_type_v'><select style="width: 80px;float: left" class="form-control" id="bill_type" name="bill_type" onchange="checkCustomer(this.value)">
                	...
                	<option value="<?php echo $CUSTOMER_ID?>"><?php echo $c_customer_name?></option>
                	...
                	<td>Place</td><td id='place_v'><input type="text" class="form-control" value="<?php echo $place?>" id="place" name="place" style="width: 160px"></td><td>&nbsp;&nbsp;</td>
                	   </tr>
                </table>
                <br/>
                </div>
              ...
              <div class="box">
                ...
                $category_id="";
                $category_name="";
                $data = mysql_query("SELECT tran_id, bill_no, item_date, item_name, item_qty, item_rate, amt, item_qty_p from customerreturn where bill_no='".$_GET["bill_no"]."'");
                while($info = mysql_fetch_array( $data ))
                {
                	
                	$data1 = mysql_query("SELECT items.category,category.category_name FROM items,category where item_name='".$info["item_name"]."' and items.category=category.category_id");
                	while($info1 = mysql_fetch_array( $data1 ))
                	{
                		$category_id=$info1[0];	
                	$category_name=$info1[1];	
           		...
           		<tr>
                	...
                	<td><select class="form-control" id="item<?php echo $i?>" name="item<?php echo $i?>"><option value="<?php echo $info["item_name"]?>"><?php echo $info["item_name"]?></option></select></td><td>&nbsp;&nbsp;</td>
                	<td><input type="text" class="form-control onlyNumbers" id="qty_piece<?php echo $i?>" name="qty_piece<?php echo $i?>" value="<?php echo $info["item_qty"]?>" size="5"></td><td>&nbsp;&nbsp;</td>
                	<td><input type="text" class="form-control" onkeypress="return isNumberKey(event)" id="qty_sft<?php echo $i?>" name="qty_sft<?php echo $i?>" value="<?php echo $info["item_qty_p"]?>" size="5" onblur="checkQty(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                	<td><input type="text" class="form-control" onkeypress="return isNumberKey(event)" id="cost<?php echo $i?>" name="cost<?php echo $i?>" value="<?php echo $info["item_rate"]?>" size="5" onblur="calCost(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                	<td><input type="text" class="form-control" id="fcost<?php echo $i?>" name="fcost<?php echo $i?>" readonly="readonly" value="<?php echo $info["amt"]?>"></td><td>&nbsp;&nbsp;</td>
                	</tr>
                	<tr><td>&nbsp;</td></tr>
                	<?php $i++;
                }
          ...
          <div class="col-xs-3">
              ...
              <div class="box">
                ...
                <br/>
                  ...
                  <input type='hidden' name='count' id='count' value="<?php echo $i?>">
                  <table>
	             <tr><td>Total&nbsp;Amount*</td><td id='grand_total_v'><input type="text" onkeypress="return isNumberKey(event)" class="form-control" onfocus="finalCost()" value="<?php echo $grand_total?>" id="grand_total" name="grand_total" readonly="readonly"/></td></tr>
	             ...
	             <tr><td>&nbsp;</td></tr>
	         <tr><td>Other&nbsp;Expenses* </td><td id='other_expenses_v'><input type="text" onkeypress="return isNumberKey(event)" class="form-control" onblur="finalCost()" value="<?php echo $other_exp?>" id="other_expenses" value='0' name="other_expenses"/></td></tr>
	         <tr><td>&nbsp;</td></tr>
	               <tr><td>Total Value*</td><td id='total_v'><input type="text" onkeypress="return isNumberKey(event)" class="form-control" onfocus="finalCost()" id="total" value="<?php echo $total?>" name="total" readonly="readonly"/>
	               <tr><td>&nbsp;</td></tr>
    ...
    <input type='hidden' name='count' id='count' value="<?php echo $i?>">
                    ...
                    <div class="form-group">                  
					<input type="submit" class="btn btn-block btn-warning" value="Update Return Bill">
				  </div>                          
