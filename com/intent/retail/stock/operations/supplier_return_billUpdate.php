...
for($i=1;$i<sizeof($data1);$i++)
{
	...
	//for($j=1;$j<=sizeof($data2);$j++)
	{
		...
		$tran_id = $row['maxid'];
				...
				VALUES ('$tran_id','$bill_no','Supplier','Credit Return','$bill_date','$flower','$desc','$qty','$aqty','$cost','$vat','$disc','$tcost','$lcost','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
	       	
		
	
		}else {
			$query="update txn_bill_support set bill_date='$bill_date',particular_id='$flower',qty_in_sft='$qty',rate='$cost',amount='$tcost',landing_cost='$lcost',qty_in_piece='$aqty',description='$desc',discount='$disc',vat='$vat' where sk_tran_id='$old_tran_id'";
			mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
		}
...
$query="update txn_transaction set tran_status='Updated' where tran_type='Transporter' and tran_desc='New Bill' and transaction_ref_id='$bill_no'";
			...
			if($transporter!=0)
			{
				...
				VALUES ('$tran_id','$bill_date',now(),'0','$transport_expenses','$transporter','$bill_no','mst_bill_main','Transporter','New Bill','$bill_type','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
			}
...
$query1 ="update txn_transaction set debit='$t_credit',tran_date='$bill_date',member_id='$supplier_id' where transaction_ref_id='$bill_no' and tran_type='Supplier' and tran_desc='New Bill'";
mysql_query($query1);
...
var c_id=document.getElementById("c_id").value
	window.location="index.php?action=purchase_return_edit&c=stock&bill_no="+bill_no;
</script>