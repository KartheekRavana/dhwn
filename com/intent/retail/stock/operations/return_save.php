...
$command = "SELECT count(sk_bill_id) as maxid FROM mst_bill_main where bill_for='Supplier' and bill_type='Credit Return'";
$result = mysql_query($command, $con);
while ($row = mysql_fetch_assoc($result))
{
...
VALUES ('$bill_no','$bill_date',now(),'$s_id','','$mobile','$place','Supplier','Credit Return','$grand_total','0','0','0','0','$grand_total','0','$transport_expenses','$other_expenses','0','$hamali_expenses','$total','0','0','0','0','0','$note','0','active','$bill_no_s','0','0','0','0','0','0','0','0','$session_id','$session_branch','0')";
...
for($i=1;$i<sizeof($data1);$i++)
{
	...
	//$total=$info['supplier_quantity']*$info['rateper_no'];
		...
		$tran_id = $row['maxid'];
				...
				VALUES ('$tran_id','$bill_no','Supplier','Credit Return','$bill_date','$flower','$desc','$aqty','$qty','$cost','$vat','$discount','$tcost','$lcost','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
...
VALUES ('$tran_id','$bill_date',now(),'0','$bal','$s_id','$bill_no','mst_bill_main','Supplier','Return Bill','Credit Return','active','$session_id','$session_branch')";
mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
...
if($transport_expenses!=0)
			{
				$tran_id++;
				$query="INSERT INTO txn_transaction(sk_tran_id,tran_date,tran_time, credit, debit,member_id, transaction_ref_id, ref_table, tran_type, tran_desc, note,tran_status, employee_id, branch_id)
				VALUES ('$tran_id','$bill_date',now(),'$transport_expenses','0','$transporter','$bill_no','mst_bill_main','Transporter','Return Bill','$bill_type','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
			}
...
if($hamali_expenses!=0)
			{
				$tran_id++;
				$query="INSERT INTO txn_transaction(sk_tran_id,tran_date,tran_time, credit, debit,member_id, transaction_ref_id, ref_table, tran_type, tran_desc, note,tran_status, employee_id, branch_id)
				VALUES ('$tran_id','$bill_date',now(),'$hamali_expenses','0','$hamali','$bill_no','mst_bill_main','Hamali','Return Bill','$bill_type','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
			}
?>
<style>
...
//
 ...
 setTimeout(function() {window.location="index.php?action=return_supplier&c=stock&status=Stock Return Succesfully";}, 2000);
</script>