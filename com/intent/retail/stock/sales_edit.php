...
function loadItems(row,val)
{
	...
	var xmlhttp;   
	 ...
	 xmlhttp.open("GET",D_PATH+"/stock/load/getItems.php?category="+category,true);
	 xmlhttp.send();
 ...
 <script type="text/javascript">
	...
	function getCustomer(customer_id)
	{
		...
		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		 ...
		 var D_PATH=document.getElementById("D_PATH").value
			var DIR=document.getElementById("DIR").value
		xmlhttp.open("GET",D_PATH+"/stock/load/customerdetails.php?customer_id="+customer_id,true);
		xmlhttp.send();
	...
	function getBarItem(row)
	{
		...
		//  document.getElementById("item"+row).className="form-control";
		 ...
		 var D_PATH=document.getElementById("D_PATH").value
			var DIR=document.getElementById("DIR").value
		xmlhttp.open("GET",D_PATH+"/stock/load/getItems.php?barcode="+barcode+"&category=0",true);
		xmlhttp.send();
		}
...
function getAllItem(row)
{
	...
	var xmlhttp;   
	 ...
	 xmlhttp.open("GET",D_PATH+"/stock/load/getAllItems.php",true);
	 xmlhttp.send();
		...
		function copyToBill(row)
		{
			 ...
			 var new_cat=document.getElementById("category"+row).value;
	           ...
	           for(var i=1;i<count;i++)
	           {
	       	 ...
	       	 var D_PATH=document.getElementById("D_PATH").value
	       		var DIR=document.getElementById("DIR").value
	       		xmlhttp.open("GET",D_PATH+"/stock/load/getItemsByDesc.php?barcode=0&category=0&item_id="+new_item+"&desc="+temp1[0],true);
...
<!-- ---------------------------------------------------POPUP--------------------------------------------------------------------------- -->  
      ...
      <div class="content-wrapper">
        <!-- Content Header (Page header) -->
          ...
          <form role="form" onsubmit="return validate()" name='myForm' action="index.php?action=operations/customerbillupdate&c=stock" method="POST">
          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="box box-primary">
                ...
                $customer_name_cash=$info["member_name"];
                	$mobile=$info['mobile'];
                	$city=$info['place'];
                ...
                <table>
                <tr>
                	<td></td><td id='bill_no_v'><input type="hidden" class="form-control onlyNumbers" id="bill_no" name="bill_no" value="<?php echo $bill_no?>"></td><td>&nbsp;&nbsp;</td>
                	<td>Bill&nbsp;Date</td><td id='bill_date_v'><input type="date" style="width: 140px" class="form-control" id="bill_date" name="bill_date" value="<?php echo $bill_date?>"></td><td>&nbsp;&nbsp;</td>
                	<td>Bill&nbsp;Type</td><td id='bill_type_v'><select style="width: 80px;float: left" class="form-control" id="bill_type" name="bill_type" onchange="checkCustomer(this.value),finalCost()"><option value="<?php echo $customer_type?>"><?php echo $customer_type?></option><option value="Cash">Cash</option><option value="Credit">Credit</option></select>
                	...
                	<?php $status="";$status1="";
                	if($supplier_id=="0"){$status=" style='visibility: hidden;position: absolute ;width: 150px'";}
                	else{$status1=" style='visibility: hidden;position: absolute ;width: 150px'";}
                	if($supplier_id==0){$supplier_id="";}
                	?>
                	<span id="customer_type" style="position: relative  ;" <?php echo $status1?>>
                	<input type="text" class="form-control" id="customer_name" name="customer_name" value='<?php echo $customer_name_cash?>'>
                	</span>
                	<span  <?php echo $status?> id="customer_type1">
                ...
                <table style="float: right;"><tr>
                	...
                	<select  name="agent_id" id="agent_id" class="form-control selectpicker pull-left" data-live-search="true" data-live-search-style="begins" title="Please select a Agent ..." style="width: 150px"  onchange="getAgent(this.value)">
                	 <?php 
                                  ...
                                  $data = mysql_query("SELECT sk_member_id,member_name FROM mst_member where member_status='active' and branch_id='$session_branch' and member_type='5' and sk_member_id='$agent_id' order by member_name asc");
                                  while($info = mysql_fetch_array( $data ))
                                  {
                                  ?>  
	               <option  value="<?php echo $info["sk_member_id"]?>"><?php echo $info["member_name"]?></option>
	               ...
	               <?php }?>
                	...
                	<input type="text" class="form-control pull-left" id="agent_name" value="" name="agent_name" style="width: 150px">
                	</span>
                	<input type="text" class="form-control pull-left" id="agent_per" name="agent_per" value="<?php echo $agent_rate?>" placeholder="%" style="width: 80px">
                	</td>
              ...
              <div class="box">
	           ...
	           $data = mysql_query("SELECT sk_tran_id, bill_id, bill_for, bill_type, bill_date, particular_id,description, qty_in_piece, qty_in_sft, rate,vat,discount, amount,landing_cost, bill_status, employee_id, branch_id,note FROM txn_bill_support where bill_id='".$bill_no."' and qty_in_sft>0");
	           while($info = mysql_fetch_array( $data ))
	           {
               ...
               <input type='hidden' id="piece_sft_<?php echo $i?>">
                ...
                <td><select class="form-control" style="padding: 0;width: 110px" id="category<?php echo $i?>" onchange="loadItems(<?php echo $i?>,this.value)"><option value="<?php echo $category_id?>"><?php echo $category_name?></option><option value=""></option><?php $data1 = mysql_query("SELECT category_id,category_name FROM category where category_status='active' and branch='".$session_branch."'");if($data1)while($info1 = mysql_fetch_array( $data1 )){?><option  value="<?php echo $info1["category_id"]?>"><?php echo $info1["category_name"]?></option><?php }?></select></td><td>&nbsp;&nbsp;</td>
                <td><select class="form-control" onchange="copyDesc(<?php echo $i?>)" style="padding: 0;width: 350px" id="item<?php echo $i?>"><option value="<?php echo $item_id?>"><?php echo $item_name?> (<?php echo $description?>)</option><option value=""></option></select></td><td>&nbsp;&nbsp;</td>
                <td><input type='text' class="form-control" value='<?php echo $item_note?>' id='note<?php echo $i?>' size="12px" style="padding: 0;"></td>
                <td><input type="text" class="form-control onlyNumbers" value="<?php echo $item_qty_p?>" style="padding: 0;" id="qty_piece<?php echo $i?>" onblur="calLength(<?php echo $i?>);totalSft(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" value="<?php echo $item_qty?>" onkeypress="return isNumberKey(event)" id="qty_sft<?php echo $i?>" onfocus="totalSft(<?php echo $i?>)" size="5" onblur="checkQty(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" value="<?php echo $item_rate?>" onkeypress="return isNumberKey(event)" id="cost<?php echo $i?>" size="5" onblur="calCost(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" value="<?php echo $item_discount?>" onkeypress="return isNumberKey(event)" id="discount<?php echo $i?>" size="5" onblur="calCost(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" value="<?php echo $amt?>" id="fcost<?php echo $i?>" readonly="readonly"></td>
                	...
                	if($k>2)
{
	$k++;
                ...
                <td><input type="text" class="form-control" id="barcode<?php echo $i?>" style="padding: 0;width: 80px" onblur="getBarItem(<?php echo $i?>)"></td><td></td>
                <td><select class="form-control" style="padding: 0;width: 110px" id="category<?php echo $i?>" onchange="loadItems(<?php echo $i?>,this.value)"><option value=""></option><?php $data = mysql_query("SELECT category_id,category_name FROM category where category_status='active' and branch='".$session_branch."'");if($data)while($info = mysql_fetch_array( $data )){?><option  value="<?php echo $info["category_id"]?>"><?php echo $info["category_name"]?></option><?php }?></select></td><td>&nbsp;&nbsp;</td>
                <td><select class="form-control" onchange="copyDesc(<?php echo $i?>)" style="padding: 0;width: 350px" id="item<?php echo $i?>"><option value=""></option></select></td><td>&nbsp;&nbsp;</td>
                <td><input type='text' class="form-control" id='note<?php echo $i?>' size="12px" style="padding: 0;"></td>
                <td><input type="text" class="form-control onlyNumbers" style="padding: 0" id="qty_piece<?php echo $i?>"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" onkeypress="return isNumberKey(event)" id="qty_sft<?php echo $i?>" onblur="checkQty(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control"  style="padding: 0" onkeypress="return isNumberKey(event)" id="cost<?php echo $i?>" onblur="calCost(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" onkeypress="return isNumberKey(event)" id="discount<?php echo $i?>" size="5" onblur="calCost(<?php echo $i?>)"></td><td>&nbsp;&nbsp;</td>
                <td><input type="text" class="form-control" style="padding: 0" id="fcost<?php echo $i?>" readonly="readonly"></td><td>&nbsp;&nbsp;</td>
          ...
          <div class="col-xs-3">
             <div class="box">
                ...
                <br/>
                  ...
                  <input type='hidden' name='count' id='count' value="<?php echo $i?>">
                  <table>
	             <tr><td>Total&nbsp;Amount*</td><td id='grand_total_v'><input type="text" value="<?php echo $grand_total-$transport_amount?>" onkeypress="return isNumberKey(event)" class="form-control" onfocus="finalCost()" id="grand_total" name="grand_total" readonly="readonly"/></td></tr>
	             ...
	             <tr><td>Transport*</td><td><select class="form-control" id="transporter" name="transporter">
	               ...
	               <tr><td>&nbsp;</td></tr>
	             <tr><td>Transport&nbsp;Amount*</td><td id='transport_v'><input type="text" value="<?php echo $transport_amount?>" onkeypress="return isNumberKey(event)" class="form-control"  onblur="finalCost()" id="transport" name="transport"/></td></tr>
	         ...
	         <tr><td>&nbsp;</td></tr>
	               <tr><td>Total Value*</td><td id='total_v'><input type="text" value="<?php echo $grand_total?>" onkeypress="return isNumberKey(event)" class="form-control" onfocus="finalCost()" id="total" name="total" readonly="readonly"/>
	               <tr><td>&nbsp;</td></tr>
	             
	             <tr><td>Note*</td><td><input type="text" class="form-control" value='<?php echo $note?>' id="note" name="note"/></td></tr> 
	            <tr><td>&nbsp;</td></tr>
                  ...
                  <input type='hidden' name='count' id='count' value="<?php echo $i?>">
                  <table>
                   <tr><td>Cash&nbsp;Amount*</td><td id='cash_amount_v'><input type="text" value="<?php echo $cash_amount?>" onkeypress="return isNumberKey(event)" onblur="cashe()" class="form-control" onfocus="finalCost()" id="cash_amount" name="cash_amount"/></td></tr>
	             <tr><td>&nbsp;</td></tr>
	             <tr><td>Check&nbsp;Amount*</td><td id='check_amount_v'><input type="text" value="<?php echo $check_amount?>" onkeypress="return isNumberKey(event)" class="form-control"  onblur="unHide();Paid()" id="check_amount" name="check_amount"/></td></tr>
	             <tr><td>&nbsp;</td></tr>
	             
                    <tr><td>Paid&nbsp;Amount*</td><td id='paid_amt_v'><input type="text" value="<?php echo $paid_amount?>" onkeypress="return isNumberKey(event)" readonly="readonly" class="form-control" onfocus="Paid()" id="paid_amt" name="paid_amt"/></td></tr>
	                <tr><td>&nbsp;</td></tr>
	                 <tr><td>Discount*</td><td id='discount_v'><input type="text" value="<?php echo $bill_discount?>" onkeypress="return isNumberKey(event)" class="form-control" onblur="Balance()" id="discount" name="discount"/></td></tr>
	             <tr><td>&nbsp;</td></tr>
	             <tr><td>Balance&nbsp;Amount*</td><td id='balance_v'><input type="text" value="<?php echo $balance_amount?>" class="form-control" readonly="readonly" id="balance" name="balance"/></td></tr>
	             <tr><td>&nbsp;</td></tr>	             
	            ...
	            <tr><td>&nbsp;</td></tr>
                  ...
                  <!-- /.box-body -->
                    ...
                    <div class="form-group">                  
					<input type="submit" class="btn btn-block btn-success" value="Update Bill" onclick="Balance()">
				  </div>             
