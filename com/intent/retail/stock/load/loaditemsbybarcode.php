...
{
echo "<table> <tr>";
	$data = mysql_query("SELECT distinct(description) as description FROM txn_bill_support WHERE particular_id=$item_id order by description");
	while($info = mysql_fetch_array($data))
	{
		$tran_id="";
		
		$data1 = mysql_query("SELECT tran_id FROM txn_bill_support WHERE particular_id=$item_id and description='".$info["description"]."' limit 1");
		while($info1 = mysql_fetch_array($data1))
		{
			$tran_id=$info1["tran_id"];
		}
...
echo "<table> <tr>";
	...
	$data = mysql_query("SELECT distinct(description) as description FROM txn_bill_support WHERE particular_id=$item_id and description like '%".$filter."%' order by description");
	while($info = mysql_fetch_array($data))
	{
	$tran_id="";
		
		$data1 = mysql_query("SELECT sk_tran_id FROM txn_bill_support WHERE particular_id=$item_id and description='".$info["description"]."' limit 1");
		while($info1 = mysql_fetch_array($data1))
		{
