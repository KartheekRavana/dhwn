 ...
 <?php include_once "$D_PATH/include/scripts_top.php";?>
  ...
  <div class="content-wrapper">
    ...
    <?php include_once "$D_PATH/include/navigation.php";?>
        ...
        <div class="row">
           <form role="form" name="myform" action="index.php?action=operations/measurement_update&c=inventory" method="POST">
          <input type='hidden' value='0' name='advance_amount'>
          <input type='hidden' id='data' name='data'>
           <input type="hidden" name="employee" value="<?php echo $session_id?>">
          <input type='hidden' id='mid' name='mid' value="<?php echo $_GET["mid"]?>">
           <input type='hidden' name="session_branch" id="session_branch" value="<?php echo $session_branch?>">
            <!-- left column -->
              ...
              <div class="box">
                ...
                <div class="box-body" >
          <?php $mobile="";$measurement_type="";
          $data = mysql_query("SELECT sk_tran_id, tran_date, tran_time, mobile,measurement_type FROM mst_measurement where sk_tran_id='".$_GET["mid"]."'");
          while($info = mysql_fetch_array( $data )){
			$mobile=$info["mobile"];
			$measurement_type=$info["measurement_type"];
			}
          ?>
          <table><tr><td>Customer Mobile</td><td><input type="text" class='form-control' value="<?php echo $mobile?>" name="mobile" id="mobile" required="required"></td><td>
          <input type="text" name="divby" class='form-control' value="144" style='width:50px' id="divby" required="required" value='144' onblur="calSft()"></td>
  <td><select name="mtype" id="mtype"><option value="<?php echo $measurement_type?>"><?php echo $measurement_type?></option><option value="Sales">Sales</option><option value="Return">Return</option></select></td>
  <td><input type='button' onclick="calSft(1)" value='Cal'></td></tr>          </table>
          </div>
          </div>
          </div>
                 </form>
	...
	<?php $items=""; 
	$i=1;
$data = mysql_query("SELECT txn_id, tran_id, category_id, item_id, length, width, div_by, total_sft, txn_status FROM txn_measurement where tran_id='".$_GET["mid"]."'");
while($info = mysql_fetch_array( $data )){

	$item_id=$info["item_id"];$item_name="";
...
?>
	<tr>
		<td><?php echo $i?></td>
		<td><select name="item<?php echo $i?>" class='select2 form-control' id="item<?php echo $i?>" style="width: 200px"><option value="<?php echo $item_id?>"><?php echo $item_name?></option><?php $data1 = mysql_query("SELECT sk_particular_id, particular_name,category_id FROM mst_particular where particular_status='active' and category_id!=0 and branch_id='".$session_branch."'");while($info1 = mysql_fetch_array( $data1 )){?><option  value="<?php echo $info1["sk_particular_id"]?>"><?php echo $info1["particular_name"]?></option><?php }?></select></td>
		<td><input type="text" class="form-control" name="length<?php echo $i?>" value="<?php echo $info["length"]?>" size=5 id="length<?php echo $i?>" onblur="calDft(<?php echo $i?>)"></td>
		<td><input type="text" class="form-control" name="width<?php echo $i?>" value="<?php echo $info["width"]?>" size=5 id="width<?php echo $i?>" onblur="calSft(<?php echo $i?>)"></td>
		<td><input type="text" class="form-control" name="sft<?php echo $i?>" value="<?php echo $info["total_sft"]?>" size=5 id="sft<?php echo $i?>"></td>
		</tr>
	<?php $i++;
}
...
<input type='hidden' name='count' id='count' value="<?php echo $i?>"> -->
          ...
          <div class="col-md-6" style="height: 200px;overflow: scroll;">
              ...
              <div class="box">
                ...
                <div class="box-body">
                 <input type="button" value="Update" class='btn btn-success' onclick="validate()">
                </div>
                </div>
