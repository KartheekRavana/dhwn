...
VALUES ('$bill_no','$bill_date',now(),'$c_id','$customer_name','$mobile','$place','$bill_for','$bill_type Return','$grand_total','$tax_rate','$tax','$tax_disc','$tax_disc_amt','$actual_amt','$transporter_id','$transport','$other_expenses','$total','$cash_amount','$check_amount','$paid_amt','$discount','$balance','$note','$bank_id','active','$m_id','$slip_no','$agent_id','$agent_per','$agent_amount','$session_id','$session_branch')";
...
$log_id=0;$command = "SELECT MAX(sk_log_id) as maxid FROM txn_log";$result = mysql_query($command, $con);while ($row = mysql_fetch_assoc($result)){$log_id = $row['maxid'];}$log_id++;
$query ="INSERT INTO txn_log (sk_log_id, tran_date, tran_time, member_type, tran_type, tran_desc, tran_table, member_id, bill_id, tran_id, branch_id, employee_id)
VALUES ('$log_id','$date',now(),'Supplier','Return','$message','txn_transaction','$c_id','$bill_no','$tran_id','$session_branch','$session_id')";
mysql_query($query);
//**********************************LOG END****************************************************
...
setTimeout(function() { window.location="?action=purchase_return&c=inventory"; }, 100);
