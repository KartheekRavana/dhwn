...
if($bill_type=="Credit")
{
	if($customer_name!="" && $c_id=="")
	{
		...
		VALUES ('$log_id','$date',now(),'Supplier','New Supplier','New Supplier Added','mst_member','$cNo','0','0','$session_branch','$session_id')";
		mysql_query($query);		
		//**********************************LOG END****************************************************
		$customer_name="";
		$c_id=$cNo;
...
VALUES ('$bill_no','$bill_date',now(),'$c_id','$customer_name','$mobile','$place','$bill_for','$bill_type','$grand_total','$tax_rate','$tax','$tax_disc','$tax_disc_amt','$actual_amt','$transporter_id','$transport','$other_expenses','$hamali_id','$hamali','$total','$cash_amount','$check_amount','$paid_amt','$discount','$balance','$note','$bank_id','active','$m_id','$agent_id','$agent_per','$agent_amount','$cmeasurement','$purchase_exp','$invoice_amt','$vat_rate','$vat','$session_id','$session_branch','$c_slip')";
...
$command = "SELECT MAX(sk_tran_id) as maxid FROM txn_bill_support";
		...
		$tran_id = $row['maxid'];
				...
				VALUES ('$tran_id','$bill_no','$bill_for','$bill_type','$bill_date','$flower','$aqty','$qty','$cost','$tcost','$lcost','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
...
mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
			...
			if($hamali_id!=0)
			{
				$tran_id++;
				$query="INSERT INTO txn_transaction(sk_tran_id,tran_date,tran_time, credit, debit,member_id, transaction_ref_id, ref_table, tran_type, tran_desc, note,tran_status, employee_id, branch_id)
				VALUES ('$tran_id','$bill_date',now(),'$hamali','0','$hamali_id','$bill_no','mst_bill_main','Transporter','New Bill','$bill_type','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
			}
			if($agent_per>0)
			{
...
$sms=$message;
 $query1="update mst_measurement set tran_status='Billed',bill_no='$bill_no' where sk_tran_id='$m_id'";
mysql_query($query1) or die('Query "' . $query1 . '" failed: ' . mysql_error());
...
$log_id=0;$command = "SELECT MAX(sk_log_id) as maxid FROM txn_log";$result = mysql_query($command, $con);while ($row = mysql_fetch_assoc($result)){$log_id = $row['maxid'];}$log_id++;
$query ="INSERT INTO txn_log (sk_log_id, tran_date, tran_time, member_type, tran_type, tran_desc, tran_table, member_id, bill_id, tran_id, branch_id, employee_id)
VALUES ('$log_id','$date',now(),'Supplier','New','$message','txn_transaction','$c_id','$bill_no','$tran_id','$session_branch','$session_id')";
mysql_query($query);
//**********************************LOG END****************************************************
...
setTimeout(function() { window.location="?action=purchase&c=inventory"; }, 100);
