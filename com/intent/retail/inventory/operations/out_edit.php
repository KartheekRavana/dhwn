...
if($bill_type=="Credit")
{
	if($customer_name!="" && $c_id=="")
	{
		...
		VALUES ('$log_id','Customer','New Customer Added','$date',now(),'$cNo','$cNo','mst_member','$session_branch','$session_id')";
		mysql_query($query);		
		//**********************************LOG END****************************************************
		$customer_name="";
		$c_id=$cNo;
...
if($agent_per!=""){
	if($agent_type=="New"){
		...
		$log_id=0;$command = "SELECT MAX(sk_log_id) as maxid FROM txn_log";$result = mysql_query($command, $con);while ($row = mysql_fetch_assoc($result)){$log_id = $row['maxid'];}$log_id++;
		$query ="INSERT INTO txn_log (sk_log_id, tran_type, tran_desc,log_date, log_time, tran_id, transaction_id, tran_table, branch_id, employee_id)
		VALUES ('$log_id','Agent','New Agent Added','$date',now(),'$agent_id','$agent_id','mst_member','$session_branch','$session_id')";
		mysql_query($query);
		//**********************************LOG END****************************************************
	}
...
VALUES ('$log_id','$date',now(),'Customer','Update','$message','txn_transaction','$c_id','$bill_no','$tran_id','$session_branch','$session_id')";
mysql_query($query);
//**********************************LOG END****************************************************
...
setTimeout(function() { window.location="?action=sales_edit&c=inventory&bill_no=<?php echo $bill_no?>"; }, 100);
