...
$query="update mst_measurement set mobile='$mobile',measurement_type='$mtype' where sk_tran_id='$mid'";
mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());

//$count=$_POST["count"];

mysql_query("delete FROM txn_measurement where tran_id='$mid'");
...
while ($row = mysql_fetch_assoc($result)){
	...
	VALUES ('$txn_id','$mid','$category_id','$item','$length','$width','$divby','$sft','pending','$employee','$branch_id')";
	mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
...
VALUES ('$log_id','$date',now(),'Customer','Measurement Update','$message','txn_transaction','0','0','$bill_no','$session_branch','$session_id')";
mysql_query($query);
//**********************************LOG END****************************************************
?>

<script>
window.location="?action=measurement_edit&c=inventory&mid=<?php echo $mid?>&status=success//Successfully Updated Measurements";
</script>
