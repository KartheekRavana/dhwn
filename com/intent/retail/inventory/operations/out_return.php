...
if($transporter_rate==""){$transporter_rate=0;}
...
$command = "SELECT count(slip_no) as maxid FROM mst_bill_main where bill_type like '%ret%' and bill_for='Customer'";
$slip_no=0;
$result = mysql_query($command, $con);
while ($row = mysql_fetch_assoc($result))
{
...
$query="INSERT INTO mst_bill_main(sk_bill_id, bill_date, bill_time, member_id, member_name, mobile, place, bill_for, bill_type, bill_amount, tax_rate, tax_amount, t_discount_rate, t_discount_amount, bill_tax_amount, transporter_id, transporter_rate, transport_amount, other_expenses, grand_total, cash_amount, check_amount, paid_amount, discount, balance_amount, note, bank_id, bill_status, measurement_slip_no,slip_no,agent_id,agent_rate,agent_amount, employee_id, branch_id)
VALUES ('$bill_no','$bill_date',now(),'$c_id','$customer_name','$mobile','$place','$bill_for','$bill_type Return','$grand_total','$tax_rate','$tax','$tax_disc','$tax_disc_amt','$actual_amt','$transporter_id','$transporter_rate','$transport','$other_expenses','$total','$cash_amount','$check_amount','$paid_amt','$discount','$balance','$note','$bank_id','active','$m_id','$slip_no','$agent_id','$agent_per','$agent_amount','$session_id','$session_branch')";
...
$command = "SELECT MAX(sk_tran_id) as maxid FROM txn_bill_support";
		...
		$tran_id = $row['maxid'];
				...
				VALUES ('$tran_id','$bill_no','$bill_for','$bill_type Return','$bill_date','$flower','$aqty','$qty','$cost','$tcost','active','$session_id','$session_branch')";
				mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
...
VALUES ('$tran_id','$bill_date',now(),'$total','$paid_amt','$c_id','$bill_no','mst_bill_main','$bill_for','Return Bill','$bill_type','active','$session_id','$session_branch')";
mysql_query($query) or die('Query "' . $query . '" failed: ' . mysql_error());
...
$log_id=0;$command = "SELECT MAX(sk_log_id) as maxid FROM txn_log";$result = mysql_query($command, $con);while ($row = mysql_fetch_assoc($result)){$log_id = $row['maxid'];}$log_id++;
$query ="INSERT INTO txn_log (sk_log_id, tran_date, tran_time, member_type, tran_type, tran_desc, tran_table, member_id, bill_id, tran_id, branch_id, employee_id)
VALUES ('$log_id','$date',now(),'Customer','Return','$message','txn_transaction','$c_id','$bill_no','$tran_id','$session_branch','$session_id')";
mysql_query($query);
//**********************************LOG END****************************************************
...
setTimeout(function() { window.location="?action=sales_return&c=inventory"; }, 100);
