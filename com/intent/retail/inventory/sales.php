 ...
 <?php include_once "$D_PATH/include/scripts_top.php";?>
  ...
  <div class="content-wrapper">
    ...
    <?php include_once "$D_PATH/include/navigation.php";?>
        ...
        <form role="form" onsubmit="return validate()" action="?action=operations/out_save&c=inventory" method="POST">
          <input type='hidden' value='0' name='advance_amount'>
          <input type='hidden' id='data' name='data'>
          <input type='hidden' id='total_sft' name='total_sft'>
          <input type='hidden' id='bill_for' name='bill_for' value="Customer">
...
$c_slip++;
                 ...
                 if(isset($_GET["m"]))
                 {
                 	...
                 	$data = mysql_query("SELECT sk_member_id,member_name,place FROM mst_member where member_type='2' and (mobile='$mobile' or landline='$mobile')");
                                  while($info = mysql_fetch_array( $data ))
                                  {
                                  	$mname=$info["member_name"];
                ...
                <table>
                <tr>
                	<td>Slip No</td><td id='bill_no_v'><input type="text" class="form-control" onfocus="finalCost()" id="slip_no" name="slip_no" value="<?php echo $c_slip?>" style="width: 150px;float: left"/></td><td>&nbsp;&nbsp;</td>
                	...
                	<td>Bill&nbsp;Type</td><td id='bill_type_v'><select style="width: 80px;float: left" class="form-control" id="bill_type" name="bill_type" onchange="checkCustomer(this.value)">
                	<option value="<?php echo $t?>"><?php echo $t?></option>
                	<option value="Cash">Cash</option><option value="Credit">Credit</option></select>
                	...
                	<option value=""></option>
                	 <?php 
                                  ...
                                  $data = mysql_query("SELECT sk_member_id,member_name FROM mst_member where member_status='active' and member_type='5' order by member_name asc");
                                  while($info = mysql_fetch_array( $data ))
                                  {
                                  ?>  
                ...
                <tr><td>&nbsp;</td></tr>
                 <tr>
                	...
                	<span id="customer_type" style='<?php echo $cash_display?>'>
                	<input type="text" class="form-control" id="customer_name" name="customer_name" style="width: 150px">
                	</span>
                	<span style="<?php echo $credit_display?>" id="customer_type1">
                	<select  name="customer_id" id="customer_id" onchange="getCustomer(this.value)" class="select2  form-control selectpicker" data-live-search="true" data-live-search-style="begins" title="Please select a customer ..." style="width: 150px">
                	<option value="<?php echo $member_id?>"><?php echo $mname?></option>
                	 <?php 
                                   
                                  $data = mysql_query("SELECT sk_member_id,member_name FROM mst_member where branch_id='$session_branch' and member_type='2' order by member_name asc");
                                  while($info = mysql_fetch_array( $data ))
                                  {
                                  ?>  
	               <option  value="<?php echo $info["sk_member_id"]?>"><?php echo $info["member_name"]?></option>
	               <?php }?>
                	...
                	<td>Place</td><td id='place_v'><input type="text" value="<?php echo $place?>" class="form-control" id="place" name="place" style="width: 160px"></td><td>&nbsp;&nbsp;</td>
              ...
              <div class="box" style="overflow: auto;max-height: 400px">
                ...
                {$item_id="";
	                ...
	                <td><select class="form-control select2" id="item<?php echo $k?>" name="item<?php echo $k?>"><option value="<?php echo $item_id?>"><?php echo $item_name?></option></select></td><td>&nbsp;&nbsp;</td>
          ...
          <div class="col-md-3">
              ...
              <div class="box">
                ...
                <br/>
	             ...
	             <tr><td>&nbsp;</td></tr>
	             <tr><td>Trans&nbsp;Amt*<input type='text' name='transporter_rate' id='transporter_rate' value="3" size='1' onblur="finalCost()"></td><td id='transport_v'><input type="text" onkeypress="return isDecimalNumber(event,this);" class="form-control"  onblur="finalCost()" id="transport" name="transport"/></td></tr>
	             <tr><td>&nbsp;</td></tr>
	             ...
	             <tr><td>&nbsp;</td></tr>	             
	               ...
	               <option value=""></option>
	                <?php 
                                  ...
                                  $data = mysql_query("SELECT sk_member_id,member_name FROM mst_member  where member_type='6' and member_status='active' order by member_name asc");
                                  if($data)
                                  while($info = mysql_fetch_array( $data ))
                                  {
                                  ?>  
 ...
 <tr><td>Note*</td><td><input type="text" class="form-control" id="note" name="note"/></td></tr>
				...
				&nbsp;
                    ...
                    <input type="button" class="btn btn-block btn-success" value="Print Preview" onclick="printPreview()">                 
					
				  </div>  
				  <div class="form-group col-md-6"> 
                   
					<input type="submit" class="btn btn-block btn-warning" value="Generate Bill">
				  </div>             
...
document.getElementById("tax").value=(grand_total*tax_rate/100).toFixed(0);
...
var transporter_rate=parseFloat(document.getElementById("transporter_rate").value).toFixed(0);
if(transporter_rate<15){
document.getElementById("transport").value=parseInt(total_sft*transporter_rate);
}else{
	document.getElementById("transport").value=transporter_rate;
}
...
function checkAgent(val)
{
 ...
 <script type="text/javascript">
		...
		function printPreview(){
			...
			var grand_total=(parseFloat(document.getElementById("grand_total").value)).toFixed(2);
			var tax=(parseFloat(document.getElementById("tax").value)).toFixed(2);
			var tax_disc_amt=(parseFloat(document.getElementById("tax_disc_amt").value)).toFixed(2);
			var total_amt=(parseFloat(document.getElementById("total_amt").value)).toFixed(2);
			var transport=(parseFloat(document.getElementById("transport").value)).toFixed(2);
			var other_expenses=(parseFloat(document.getElementById("other_expenses").value)).toFixed(2);
			...
			data="//"+bill_date+"//"+mobile+"//"+grand_total+"//"+tax+"//"+tax_disc_amt+"//"+total_amt+"//"+transport+"//"+other_expenses+"//"+total+"//"+data

			window.open("?action=print/preview_bill&c=inventory&data="+data+"&m="+<?php echo $slip_no?>, '_blank');    
			//window.location="?action=print/preview_bill&c=inventory&data="+data;
		}
        </script>
